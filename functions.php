...
function Rafael_scripts_method() {
...
add_action( 'wp_enqueue_scripts', 'Rafael_scripts_method' );
...
function custom_admin_js() {
    $custom_admin_url = get_bloginfo('template_directory') . '/js/custom-admin.js';
    echo '<script type="text/javascript" src="'. $custom_admin_url . '"></script>';
}

function custom_admin_css() {
    $custom_admin_url_css = get_bloginfo('template_directory') . '/css/custom-admin.css';
    echo '<link href="'. $custom_admin_url_css . '" rel="stylesheet" />';
}
...
wp_enqueue_script( 'custom', '/wp-content/themes/santissima/js/custom.js', array('jquery'));
//wp_enqueue_script( 'akna', '/wp-content/themes/santissima/js/akna.js');
...
add_action( 'woocommerce_register_form_start', 'wooc_extra_register_fields', 0);
...
add_action( 'woocommerce_created_customer', 'wooc_save_extra_register_fields', 0);





function santissima_enqueue_script(){
	$params = array(
	...
	if(is_checkout()){
		wp_enqueue_style('checkout-santissima', get_template_directory_uri() . '/css/checkout.css', array('porto-theme-elements', 'porto-theme', 'porto-theme-shop'));
		//wp_enqueue_script('santissimajs_checkout', get_template_directory_uri() . '/js/santissimajs_checkout.js', array( 'jquery', 'pagseguro-library', 'pagseguro-checkout', 'woocommerce-extra-checkout-fields-for-brazil-front' ));
		
		wp_localize_script( 'santissimajs_checkout', 'defs', $params );
		wp_enqueue_script('santissimajs_checkout');
	}
	
	if(is_product()){
		wp_enqueue_script('santissimajs_cart', get_template_directory_uri() . '/js/santissimajs_cart.js', array( 'jquery' ));
		
		wp_localize_script( 'santissimajs_cart', 'defs', $params );
		wp_enqueue_script('santissimajs_cart');
		
		//Alteração de foto no mobile
		wp_enqueue_style('santissima_product_photo_css', get_template_directory_uri() . '/css/custom_single_product.css', array('porto-bootstrap'));
		
		
	}

    if(is_product_category() || is_search()){
        wp_enqueue_style('santissima_category_custom', get_template_directory_uri() . '/css/custom_category.css', array('porto-bootstrap'));
    }
}
add_action ( 'wp_enqueue_scripts', 'santissima_enqueue_script' );
...
add_action( 'wp_ajax_nopriv_check_date_birthday', 'is_valid_nasc' );
add_action('wp_ajax_check_date_birthday', 'is_valid_nasc');
...
function custom_tracking( $order_id ) {
    ...
    'sku': '<?php echo $item['product_id']; ?>', // código do produto
    'category': 'Roupas', // aplicar conforme categoria do produto
    'price': '<?php echo $order->get_line_subtotal( $item )/$item['qty']; ?>', // preço unitário do produto
    'quantity': '<?php echo $item['qty']; ?>' // quantidade de unidades do produto na transação
    });
    <?php } ?>

    //repetir código gtm_products.push(...) para cada produto do pedido.
    
    dataLayer.push({
    'event': 'trackTrans', // evento que será regra para tag
    'transactionId': '<?php echo $order->get_order_number(); ?>', // código do pedido
    'transactionAffiliation': '<?php echo get_bloginfo( 'name' ); ?>', // nome da loja
    'transactionTotal': '<?php echo $order->get_subtotal(); ?>', // valor total da transação
    'transactionTax': '<?php echo $order->get_total_tax(); ?>', // valor numérico de imposto
    'transactionShipping': '<?php echo $order->get_total_shipping(); ?>', // valor numérico de imposto
    'transactionProducts': gtm_products // lista de produtos anterior
    });
    </script>

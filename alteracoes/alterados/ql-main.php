							...
							switch ( $type ) :
								...
								case 'color':
									...
									if ( $color =  $this->get_vas( $color_id ) ) :
										 ...
										 * @param string
										 */
										echo apply_filters( 'queryloop_visual_attributes_picker', '<a class="va-picker va-picker-color ' . esc_attr( $selected ) . '" ' . $datas . ' title="' . esc_attr( $this->get_attribute_name( $attribute_attr, $term_raw ) ) . '"><span class="va-picker-item va-color" ' . $color_style . '></span>' . $this->display_name( $attribute_attr, $post_id, $term_raw ) . '</a>');
									endif;
									break;
